    ...
    for person_dir in os.scandir(args["directory"]):
        if person_dir.is_dir():
            for file in os.scandir(person_dir.path):
                if file.is_file() and (file.path.endswith(".jpg") or file.path.endswith(".png")):
...
def load_local_image_to_df(img_source, target_name):
    img_array = cv2.imread(img_source, cv2.IMREAD_GRAYSCALE)
...
ap.add_argument("-d", "--directory",
        default="D:\TEC\Patrones\Img_PCA",
